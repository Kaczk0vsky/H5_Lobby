    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            with transaction.atomic():
                ...
                add_new_user_to_vpn_server(os.getenv("SERVER_URL"), os.getenv("VPN_SERVER_PASSWORD"), vpncmd_commands)
    ...
    def _check_if_banned(self, player):
        ...
        if ban:
            ...
            if ban_duration == timedelta(0):
                ...
                return JsonResponse({"success": False, "error": f"You are banned for {ban_duration}"}, status=400)
...
class SetPlayerStateView(View):
    ...
    def post(self, request, *args, **kwargs):
        try:
            if not self.state:
                return JsonResponse({"success": False, "error": "Invalid player state configuration"}, status=500)
    ...
    def _handle_opponent_state(self, opponent, player, player_matched):
        if opponent.player_state == "accepted" or opponent.player_state == "playing":
            ...
            with transaction.atomic():
                ...
                return JsonResponse({"success": True, "oponnent_accepted": True, "oponnent_declined": False})
            ...
            with transaction.atomic():
                ...
                return JsonResponse({"success": True, "oponnent_accepted": False, "oponnent_declined": True})
